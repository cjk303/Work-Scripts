...
def fetch_products(org: str) -> List[str]:
    return parse_name_column(hammer("product", "list", "--per-page", "999", org=org))


def fetch_repos(org: str, product: str) -> List[str]:
    return parse_name_column(
        hammer("repository", "list", "--product", product, "--per-page", "999", org=org)
    )


def fetch_content_views(org: str) -> List[str]:
    return parse_name_column(
        hammer("content-view", "list", "--per-page", "999", org=org),
